...
{
	d_zoomer_waterfall = NULL; // need this for proper init
	d_start_frequency = -1;
	d_stop_frequency = 1;
...
//	setAxisTitle(QwtAxis::YLeft, "Time (s)");
	...
	d_zoomer_waterfall = new WaterfallZoomer(canvas(), 0);
#if QWT_VERSION < 0x060000
	d_zoomer_waterfall->setSelectionFlags(QwtPicker::RectSelection | QwtPicker::DragSelection);
#endif
	d_zoomer_waterfall->setMousePattern(
				QwtEventPattern::MouseSelect2, Qt::RightButton, Qt::ControlModifier);
	d_zoomer_waterfall->setMousePattern(QwtEventPattern::MouseSelect3, Qt::RightButton);

	const QColor c(Qt::black);
	d_zoomer_waterfall->setRubberBandPen(c);
	d_zoomer_waterfall->setTrackerPen(c);
...
void WaterfallDisplayPlot::resetAxis()
{
	...
	QwtDoubleRect zbase = d_zoomer_waterfall->zoomBase();
	d_zoomer_waterfall->zoom(zbase);
					     ...
					     const std::string& strunits)
{
	...
	if (stopFreq > startFreq) {
		...
		if ((axisScaleDraw(QwtAxis::XBottom) != NULL) && (d_zoomer_waterfall != NULL)) {
			...
			((WaterfallZoomer*)d_zoomer_waterfall)->setFrequencyPrecision(display_units);
			((WaterfallZoomer*)d_zoomer_waterfall)->setUnitType(strunits);
		}
	}
}
...
void WaterfallDisplayPlot::replot()
{
	...
	if (d_zoomer_waterfall != NULL) {
		((WaterfallZoomer*)d_zoomer_waterfall)->updateTrackerText();
	}
